var profileDependencies = ['SocialNetView', 'text!templates/profile.html',
        'text!templates/status.html', 'models/Status',
        'views/Status'];
function profileDefinition (SocialNetView,  profileTemplate,
                            statusTemplate, Status, StatusView)
{
    var profileView = SocialNetView.extend({
        ...
        prependStatus: function(statusModel) {
            var statusHtml = (new StatusView({ model: statusModel }))
                    .render().el;
        ...
        render: function() {
            ...
            if ( null !== statusCollection ) {
                _.each(statusCollection, function (statusJson) {
                    ...
                    var statusHtml = (new StatusView({ model: statusModel }))
                            .render().el;
                    $(statusHtml).prependTo('.status_list').hide()
                        .fadeIn('slow');
                });
            }
        }
...
define(profileDependencies, profileDefinition);
